...
static bool check_image_valid(IMAGE* image) {
    return GetImageBuffer(image) and image->getwidth() > 0 and image->getheight() > 0;
}

ResourceManager* ResourceManager::manager = nullptr;

ResourceManager* ResourceManager::instance() {
    if (not manager) manager = new ResourceManager();
    return manager;
}
...
void ResourceManager::flip_image(IMAGE* src_image, IMAGE* dst_image, int num_h) {
    ...
    DWORD *src_buffer = GetImageBuffer(src_image), *dst_buffer = GetImageBuffer(dst_image);
    for (int i = 0; i < num_h; ++i) {
        int x_left = i * w_frame;
        int x_right = (i + 1) * w_frame;
        for (int y = 0; y < h; ++y) {
            for (int x = x_left; x < x_right; ++x) {
                int idx_src = y * w + x, idx_dst = y * w + x_right - (x - x_left);
...
void ResourceManager::flip_image(const std::string& src_id, const std::string& dst_id, int num_h) {
    IMAGE *src_image = image_pool[src_id], *dst_image = new IMAGE();
...
void ResourceManager::flip_atlas(const std::string& src_id, const std::string& dst_id) {
    Atlas *src_atlas = atlas_pool[src_id], *dst_atlas = new Atlas();
...
void ResourceManager::load() {
    // 遍历静态列表, 读取图片/图集加载到资源池中, 若路径不存在抛出异常
    for (const auto& info : image_info_list) {
        auto image = new IMAGE();
        loadimage(image, info.path);
        if (not check_image_valid(image)) throw info.path;
