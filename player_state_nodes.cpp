...
PlayerAttackStates::PlayerAttackStates() {
    timer.set_wait_time(.3f);
    timer.set_one_shot(true);
    timer.set_on_timeout([&]() {
        auto player = dynamic_cast<Player*>(CharacterManager::instance()->get_player());
        player->set_attacking(false);
    });
}

void PlayerAttackStates::on_enter() {
...
void PlayerAttackStates::on_update(float delta) {
    ...
    else if (not player->get_attacking()) {
        if (player->get_velocity().y > 0) player->switch_state("fall");
        else if (player->get_move_axis() == 0) player->switch_state("idle");
        else if (player->is_on_floor() and player->get_move_axis() != 0) player->switch_state("run");
    }
}

void PlayerAttackStates::on_exit() {
    auto player = dynamic_cast<Player*>(CharacterManager::instance()->get_player());
    player->get_hit_box()->set_enabled(false);
    player->set_attacking(false);
}

void PlayerAttackStates::update_hit_box_position() {
    auto player = dynamic_cast<Player*>(CharacterManager::instance()->get_player());
