...
BulletTimeManager* BulletTimeManager::instance() {
    if (not manager) manager = new BulletTimeManager();
    return manager;
}

void BulletTimeManager::post_process() const {
    ...
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            ...
            buffer[idx] = BGR(RGB(r, g, b)) | static_cast<DWORD>(static_cast<BYTE>(255)) << 24;
        }
    }
}
