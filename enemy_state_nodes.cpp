//
// Created by LuoYu on 2025/12/21.
//
...
EnemyAimState::EnemyAimState() {
    ...
    timer.set_on_timeout([&] {
        auto enemy = dynamic_cast<Enemy*>(CharacterManager::instance()->get_enemy());
...
void EnemyDashInAirState::on_enter() {
    ...
    const auto player = CharacterManager::instance()->get_player();
...
void EnemyDashOnFloorState::on_update(float delta) {
    ...
    if (enemy->get_hp() <= 0) enemy->switch_state("dead");
    else if (not enemy->get_dashing_on_floor()) enemy->switch_state("idle");
}
...
EnemyIdleState::EnemyIdleState() {
    ...
    timer.set_on_timeout([&] {
        ...
        if (enemy->get_hp() > 8) {
            if (rand_num <= 25) {
                if (not enemy->is_on_floor()) enemy->switch_state("fall");
                else enemy->switch_state("jump");
            } else if (rand_num <= 50) {
                if (not enemy->is_on_floor()) enemy->switch_state("fall");
                else enemy->switch_state("run");
            } else if (rand_num <= 80) enemy->switch_state("squat");
            else if (rand_num <= 90) enemy->switch_state("throw_silk");
            else enemy->switch_state("throw_sword");
        } else {
            if (rand_num <= 25) {
                if (not enemy->is_on_floor()) enemy->switch_state("fall");
                else enemy->switch_state("jump");
            } else if (rand_num <= 60) enemy->switch_state("throw_sword");
            else if (rand_num <= 70) enemy->switch_state("throw_silk");
            else if (rand_num <= 90) enemy->switch_state("throw_barb");
...
void EnemyIdleState::on_enter() {
    ...
    float wait_time = 0;
    if (enemy->get_hp() > 8) wait_time = static_cast<float>(range_random(0, 2)) * .25f;
    else wait_time = static_cast<float>(range_random(0, 1)) * .05f;
...
EnemyThrowSilkState::EnemyThrowSilkState() {
    timer.set_one_shot(true);
    timer.set_wait_time(.4f);
    timer.set_on_timeout([&] {
        auto enemy = dynamic_cast<Enemy*>(CharacterManager::instance()->get_enemy());

        enemy->set_gravity_enabled(true);
        enemy->set_throwing_silk(false);
        if (not enemy->is_on_floor() and enemy->get_hp() > 5 and range_random(0, 100) <= 25)
            enemy->switch_state("aim");
        else if (not enemy->is_on_floor()) enemy->switch_state("fall");
...
EnemyThrowSwordState::EnemyThrowSwordState() {
    ...
    timer_switch.set_wait_time(1.f);
